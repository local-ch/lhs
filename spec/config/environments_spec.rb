...
describe LHS::Config do
  ...
  context 'environments' do
    it 'provides configuration for production' do
      ...
      expect(LHS::Config[:datastore]).to eq 'http://datastore.lb-service'
    end

    it 'provides configuration for development' do
      allow(Rails).to receive(:env).and_return(ActiveSupport::StringInquirer.new('development'))
      expect(LHS::Config[:datastore]).to eq 'http://datastore-stg.lb-service.sunrise.intra.local.ch'
    end
    ...
    it 'provides configuration for all the others' do
      ...
      expect(LHS::Config[:datastore]).to eq 'http://datastore-stg.lb-service'
    end
  end
end

...
describe LHS::Service do
  ...
  context 'instance' do
    ...
    it 'is querying relevant endpoint when using where' do
      stub_request(:get, "#{LHS::Config[:datastore]}/v2/feedbacks?has_review=true").to_return(status: 200, body: '')
      SomeService.where(has_review: true)
      stub_request(:get, "#{LHS::Config[:datastore]}/v2/content-ads/123/feedbacks?has_review=true").to_return(status: 200, body: '')
    ...
    it 'is using params as query params explicitly when provided in params namespace to prevent clashing from injections' do
      stub_request(:get, "#{LHS::Config[:datastore]}/v2/content-ads/123/feedbacks?campaign_id=456").to_return(status: 200, body: '')
